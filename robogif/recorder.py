...
def check_requirements():
    ...
    try:
        ...
        if not (
                                    "format" in output and "scale" in output and "palettegen" in output and "paletteuse" in output and "fps" in output):
...
def get_chosen_device(devices):
    ...
    for device_id in devices:
        print("{t.green}[{num}] {t.white}{model} - {t.yellow}{device_id}".format(t=t, num=num, model=devices[device_id][
            "model"] if "model" in devices[device_id] else "(unknown)", device_id=device_id))
...
def create_optimized_video(in_file, out_file, size, fps, video_quality):
    ...
    FFMPEG_FILTERS = "format=pix_fmts=yuv420p,fps={fps},scale=w='if(gt(iw,ih),-2,{size})':h='if(gt(iw,ih),{size},-2)':flags=lanczos".format(
        fps=fps, size=size)
    FFMPEG_CONVERT = ["ffmpeg", "-v", "warning", "-i", in_file, "-codec:v", "libx264", "-preset", "slow", "-crf",
                      str(video_quality), "-vf", FFMPEG_FILTERS, "-y", "-f", "mp4", out_file]
...
def create_optimized_gif(in_file, out_file, size, fps):
    ...
    FFMPEG_FILTERS = "fps={fps},scale=w='if(gt(iw,ih),-1,{size})':h='if(gt(iw,ih),{size},-1)':flags=lanczos".format(
        fps=fps, size=size)
    FFMPEG_PALLETE = ["ffmpeg", "-v", "warning", "-i", in_file, "-vf", FFMPEG_FILTERS + ",palettegen", "-y",
                      tmp_pal_file]
    FFMPEG_CONVERT = ["ffmpeg", "-v", "warning", "-i", in_file, "-i", tmp_pal_file, "-lavfi",
                      FFMPEG_FILTERS + "[x];[x][1:v]paletteuse=dither=floyd_steinberg", "-y", "-f", "gif",
                      convert_output_path]
...
@click.argument("filename", type=click.Path(exists=False, writable=True, resolve_path=True),
                metavar="<filename>.<gif|mp4>")
@click.option('-i', '--input-file', type=str, help="Convert input mp4 file to optimized gif")
@click.option('-s', '--size', type=int, default=480,
              help="Size of the shortest side of the output gif/video. Defaults to 480.")
@click.option('-f', '--fps', type=int, help="Framerate of the output gif/video. Defaults to 15 for GIF and 60 for MP4.")
@click.option('-vq', '--video-quality', type=int, default=24,
              help="Video quality of the output video - the value is x264 CRF. Default is 24, lower number means better quality.")
@click.help_option()
...
def run(filename=None, input_file=None, size=None, fps=None, video_quality=None):
    ...
    if not (filename.lower().endswith(".mp4") or filename.lower().endswith(".gif")):
        ...
        print(t.red("Filename must either end with"), t.green("mp4"), t.red("for video or"), t.green("gif"),
              t.red("for a GIF."))
        print
        sys.exit(-4)
